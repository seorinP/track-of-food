...
- `/src`
> 화면을 구성하는 component들과 이를 묶어둔 pages, 클라이언트단의 정보를 저장하는 store, api관련 코드들이 든 utiles로 이루어짐
- `dockerfile`
...
**웹서비스의 해결 과제와 데이터분석으로 해결하기 위한 방안 논의 (50자 이상)**
  - 프로젝트 아이디어 동기 <br>
    : 코로나 전후의 배달 데이터 분석 결과, 코로나 확진자가 증가할 수록 배달 주문량이 증가하였다. <br>
      ...
      이에 우리는 배달 음식과 떨어진 줄어든 활동량에 의해 과섭취하게 되는 칼로리만큼을 태우기 위한 사용자 근처의 산책로를 추천하는 서비스를 기획함<br> <br>
  - 문제를 해결하기 위한 특정 질문 명시 <br>
    : 배달 음식 섭취로 인해 과섭취한 칼로리를 사용자의 위치 주변의 산책로를 추천하여 걷기 혹은 조깅으로 소모할 수 있는가?<br><br>
  - 데이터분석을 통해 해결하려는 문제를 구체적으로 작성<br>
    : 배달 음식과 현저히 줄어든 외부 활동에 의해 과섭취하게 되는 칼로리만큼을 태우기 위한 사용자 근처의 산책로를 추천.<br>
